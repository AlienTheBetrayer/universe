...
export const Tutorial = ({ pages, onSkip, onSelect }: Props) => {
    ...
    const next = () =>
        setSelected((prev) => (prev < pages.length - 1 ? prev + 1 : prev));
    ...
    return (
        <>
            ...
            >
                ...
                {/* mobile buttons */}
                <Button
                    className='tutorial-mobile-button tutorial-left'
                    enabled={selected > 0}
                    onClick={() => previous()}
                >
                    ←
                ...
                <Button
                    className='tutorial-mobile-button tutorial-right'
                    enabled={selected < pages.length - 1}
                    onClick={() => next()}
                >
                    →
                ...
                >
                    <Button
                        ...
                        onClick={() => previous()}
                        ref={(el) =>
                            tooltips.set(2, 'Previous tutorial page', el, 'up')
                        }
                    >
                        ←
                        <HotkeyTooltip
                        ...
                        <div className='tutorial-card-content'>
                            <h3
                                dangerouslySetInnerHTML={{
                                    __html: pages[selected].title,
                                }}
                            />
                            <p
                                dangerouslySetInnerHTML={{
                                    __html: pages[selected].description,
                                }}
                            />
                        </div>
                    </div>
                    ...
                    <Button
                        ...
                        onClick={() => next()}
                        ref={(el) =>
                            tooltips.set(3, 'Next tutorial page', el, 'up')
                        }
                    >
                        →
                        <HotkeyTooltip
