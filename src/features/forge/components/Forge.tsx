...
export const Forge = () => {
	...
	useEffect(() => {
		if (state.effectSlots.length >= 9) {
			analytics.current.send("effect_bonus", "toggled all 9 effects in forge");
		}
	}, [state.effectSlots, analytics]);
					...
					onSelect={() => {
						...
						setTimeout(() => {
							if (effectContainerRef.current) {
								const boundsTop =
									effectContainerRef.current?.getBoundingClientRect().top;
