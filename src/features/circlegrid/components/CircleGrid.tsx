...
export const CircleGrid = ({ controller = true }: Props) => {
    ...
    return (
        <div className='circle-grid' ref={containerRef}>
            {Array.from({ length: 8 }).map((_, idx) => (
                <motion.div
                    ...
                    layout
                    style={{
                        gridRow: grid.row(idx + 1),
                        gridColumn: grid.column(idx + 1),
                    }}
                    transition={{
                ...
                >
                    <button
                        className={`circle-grid-main-button ${idx === grid.coloredIdx ? 'circle-grid-main-button-selected' : ''}`}
                        ref={(el) =>
                            tooltips.set(
                                idx,
                                'Paint / Select this element',
                                el,
                                'up',
                            )
                        }
                    ...
                    <div
                        className={`circle-grid-dots ${idx === grid.coloredIdx ? 'circle-grid-dots-selected' : ''}`}
                    />
                </motion.div>
            ))}
